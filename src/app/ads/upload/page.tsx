...
import AdUploadData from "./upload-table-data"
...
const FileUpload = () => {
  ...
  const [uploadsData, setUploadsData] = useState<UploadAd[]>([]);
  const [filteredData, setFilteredData] = useState<UploadAd[]>([]);
  ...
  const fetchAdData = async () => {
    ...
    try {
      const response = await axiosInstance.get<UploadAd[]>("/bulk-ad-data", {
        headers: {
      ...
      if (response && response.data) {
        const processedData:UploadAd[] = response.data.map((item) => ({
          ...item,
        }));
  ...
  const handleUpload = async () => {
    ...
    try {
        const response = await axiosInstance.post('/ads/bulk-upload',formData,  {
            headers: {
                'Authorization': `Bearer ${token}`,
