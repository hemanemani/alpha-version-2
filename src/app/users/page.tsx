        ...
        cell: ({ row }) => (
          <DropdownMenu open={openId === row.original.id} onOpenChange={(isOpen) => setOpenId(isOpen ? row.original.id : null)}>
            ...
            <DropdownMenuContent align="end" className="w-52 bg-white dark:bg-[#111111] border border-[#d9d9d9] dark:border-[#2e2e2e] rounded-lg">
              <DropdownMenuItem className={`flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-white cursor-pointer  rounded-sm py-2 dark:hover:bg-[#2C2D2F] ${((row.original.id === 1) || (row.original.id === 2)) ? '' : 'border-b  border-b-[#d9d9d9] dark:border-b-[#2e2e2e]'}`} onClick={() => handleEdit(row.original.id)}>
  ...
  return (
    <div>
        <div className="ml-[85px] -mt-[30px] mb-[20px]">
          <p className="text-[28px] text-[#000] dark:text-white mt-[5px] font-inter-bold">{users.length}</p>
        </div>
        ...
        <div className="flex justify-end items-center mb-4 gap-4">
            ...
            <Link href="/users/create">
                <RainbowButton className="bg-black dark:bg-white dark:text-black text-white text-[11px] captitalize px-2 py-1 h-[37px] cursor-pointer font-inter-semibold">+ Add New User</RainbowButton>
            </Link>
        </div>
