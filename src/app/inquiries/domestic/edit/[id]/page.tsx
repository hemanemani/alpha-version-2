  ...
  if (id) {
    ...
    return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 gap-2 mb-6 mt-4">
                ...
                <SelectContent className="dark:bg-[#000]">
                  {formData.status !== 1 && !wasOfferMode ? (
                    ...
                    <SelectItem className="text-[13px] cursor-pointer dark:hover:bg-[#2C2D2F] dark:active:bg-[#2C2D2F] dark:focus:bg-[#2C2D2F]" value="select status">
                      ...
                      <SelectItem value="offer" className="text-[13px] cursor-pointer dark:hover:bg-[#2C2D2F]">Move to Offers</SelectItem>
                      <SelectItem value="cancel" className="text-[13px] cursor-pointer dark:hover:bg-[#2C2D2F]">Move to Cancel</SelectItem>
                    </>
                  ) : (
                    <>
                    <SelectItem className="text-[13px] cursor-pointer dark:hover:bg-[#2C2D2F]" value="select status">
                      Select Status
                    </SelectItem>
        ...
        <RainbowButton 
         type="submit"
         className={`${isLoading ? "opacity-50 cursor-not-allowed" : ""} w-[40%] bg-black dark:bg-[#000] text-white dark:text-black capitalize text-[15px] h-[43px] rounded-sm block ml-auto mr-auto mt-10 font-inter-semibold cursor-pointer `}
         disabled={isLoading}
         >
          {isLoading ? (
      ...
      <Dialog open={openDialog} onOpenChange={(open) => {
        ...
        <DialogContent className="gap-6 rounded-md b">
          ...
          <p className="text-[13px] text-[#7f7f7f] font-inter-medium">{conflictMobile} already exists in 
            <span dangerouslySetInnerHTML={{ __html: conflictMessage || "" }} />Do you want to edit the inquiry anyway?</p>
