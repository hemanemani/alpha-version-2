...
const InternationalInquiriesDashboard:React.FC = () => {
  ...
  const handleEdit = (id: number, serial:number) => {
            ...
            <DropdownMenu open={openId === row.original.id} onOpenChange={(isOpen) => setOpenId(isOpen ? row.original.id : null)}>
              ...
              <DropdownMenuContent align="end" className="w-52 bg-white dark:bg-[#111111] border border-[#d9d9d9] dark:border-[#2e2e2e] rounded-lg">
                ...
                <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleOffers(row.original.id)}>
                  <Move className="h-4 w-4 text-gray-600 dark:text-white" /> Move to Offers
                </DropdownMenuItem>
                <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleCancel(row.original.id)}>
                  <Ban className="h-4 w-4 text-gray-600 dark:text-white" /> Cancel
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            )
  ...
  const exportToCSV = () => {
    ...
    utils.book_append_sheet(workbook, worksheet, "International Inquiries");
    writeFile(workbook, "international-inquiries.csv");
  };
  ...
  const exportToExcel = () => {
    ...
    const workbook = utils.book_new();
    utils.book_append_sheet(workbook, worksheet, "International Inquiries");
    writeFile(workbook, "international-inquiries.xlsx");
  };
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          ...
          <RainbowButton className="bg-black text-white dark:bg-white dark:text-black text-[11px] captitalize px-2 py-1 h-[37px] cursor-pointer font-inter-semibold">+ Add New Inquiry</RainbowButton>
          </Link>
          )}
          {(accessLevel === "master") && hasAccessTo("/inquiries/international/upload") && (
          <Link href="/inquiries/international/upload">
          <Button className="bg-transparent text-black dark:text-white rounded-small text-[11px] px-2 py-1 captitalize border-2 border-[#d9d9d9] hover:bg-transparent cursor-pointer font-inter-semibold">+ Bulk Upload</Button>
          </Link>
          )}
          {accessLevel === "master" && (
          <DropdownMenu>
