...
const handleDateChange = (date: Date | undefined, field: keyof InternationalInquiryFormData) => {
      ...
      const fetchNextNumber = async () => {
        ...
        try {
          const res = await axiosInstance.get(
            '/international_inquiries/next-number',
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
    ...
    useEffect(() => {
      const fetchNextSerial = async () => {
        ...
        try {
          const response = await axiosInstance.get('/international_inquiries/next-serial-number',{
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
    ...
    <form className="px-20 py-6" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-2 mb-6 mt-4">
          ...
          <div className="space-y-2 w-[80%]">
            ...
            { isInputLoading ? <SkeletonCard height="h-[36px]" /> : <Input id="inquiryNumber" name="inquiry_number" value={formData.inquiry_number || ''} placeholder="Please enter inquiry number" onChange={handleChange} className={`bg-white dark:bg-gray-950 ${formErrors.inquiry_number ? "border-red-500" : ""}`} /> }
          </div>
          <div className="space-y-2 w-[80%]">
            <Label htmlFor="inquiryDate" className="text-[15px] font-inter-medium">Inquiry Date</Label>
            <div className={`bg-white dark:bg-gray-950 rounded-md ${formErrors.inquiry_date ? "border-red-500" : ""}`}>
            <DatePicker
              id="inquiryDate"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-2 mb-6 mt-4">
          <div className="space-y-2 w-[80%]">
            ...
            <Input id="mobileNumber" name="mobile_number" value={formData.mobile_number || ''} onChange={handleChange} placeholder="Please enter mobile number" className={`bg-white dark:bg-gray-950`}/>
