...
const CancellationsDomesticInquiriesDashboard:React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      const response = await axiosInstance.get<Inquiry[]>('/inquiry-cancellation-offers', {
  ...
  const handleBlockInquiry = async (id: number, mobile_number: string, 
    ...
    try {
      ...
      const response = await axiosInstance.post<BlockResponse>(`/block-inquiry/${id}`, { mobile_number },
        { headers: 
  ...
  },
  {
    ...
    cell: ({ row, table }) =>{
      ...
      const serial = (totalRows - index) - 1;
      ...
      <DropdownMenu open={openId === row.original.id} onOpenChange={(isOpen) => setOpenId(isOpen ? row.original.id : null)}>
        ...
        <DropdownMenuContent align="end" className="w-52 bg-white dark:bg-[#111111] border border-[#d9d9d9] dark:border-[#2e2e2e] rounded-lg">
          ...
          <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleInquiry(row.original.id)}>
            <Move className="h-4 w-4 text-gray-600 dark:text-white" /> Move back to Inquiry
          </DropdownMenuItem>
          <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleBlockInquiry(row.original.id,row.original.mobile_number)}>
  ...
  const exportToCSV = () => {
    ...
    utils.book_append_sheet(workbook, worksheet, "Cancelled Inquiries");
    writeFile(workbook, "cancelled-inquiries.csv");
  };
  ...
  const exportToExcel = () => {
    ...
    const workbook = utils.book_new();
    utils.book_append_sheet(workbook, worksheet, "Cancelled Inquiries");
    writeFile(workbook, "cancelled-inquiries.xlsx");
  };
      ...
      <div className="flex justify-between items-center p-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Select
              ...
              defaultValue="10"
            >
              <SelectTrigger className="w-[60px] h-[25px] text-[13px] p-2 font-inter-semibold cursor-pointer">
...
export default CancellationsDomesticInquiriesDashboard;
