 ...
 const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (error instanceof AxiosError) {
          ...
          console.log(error.response?.data?.errors?.mobile_number?.[0] || error.response?.data?.message);
    ...
    {/*********************************** shipping details **************************************/}
      ...
      {uniqueAssignedSellers.length > 0 && (
        ...
        <RainbowButton 
         type="submit"
         className={`${isLoading ? "opacity-50 cursor-not-allowed" : ""} w-[40%] bg-black dark:bg-white dark:text-black text-white capitalize text-[15px] h-[43px] rounded-sm block ml-auto mr-auto mt-10 font-inter-semibold cursor-pointer `}
         disabled={isLoading}
         >
          {isLoading ? (
      ...
      <DialogContent className="gap-6 rounded-md b">
        ...
        <p className="text-[13px] text-[#7f7f7f] font-inter-medium">{conflictMobile} already exists in 
          <span dangerouslySetInnerHTML={{ __html: conflictMessage || "" }} />Do you want to edit the order anyway?</p>
