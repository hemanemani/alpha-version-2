...
const InternationalOrdersDashboard:React.FC = () => {
  ...
  const columns: ColumnDef<OrderWithShipping>[] = [
    ...
    },
    {
      ...
      header: "",
      cell: ({ row }) => (
        ((accessLevel === "master") || accessLevel === "full" || (accessLevel === "limited" && hasAccessTo("/orders/international/edit")))
            && (
        <DropdownMenu open={openId === row.original.id} onOpenChange={(isOpen) => setOpenId(isOpen ? row.original.id : null)}>
          ...
          <DropdownMenuContent align="end" className="w-52 bg-white dark:bg-[#111111] border border-[#d9d9d9] dark:border-[#2e2e2e] rounded-lg">
            ...
            <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleCancel(row.original.international_offer?.international_inquiry?.id || row.original.id)}>
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          {(
          ((accessLevel === "master") || accessLevel === "full" && hasAccessTo("/orders/international/create")) ||
          (accessLevel === "limited" && allowedPages.includes("/orders/international/create"))
          )  && (
          <Link href="/orders/international/create">
          <RainbowButton className="bg-black dark:bg-white dark:text-black text-white text-[11px] captitalize px-2 py-1 h-[37px] cursor-pointer font-inter-semibold">+ Add New Order</RainbowButton>
          </Link>
          )}
        {accessLevel === "master" && (
          <DropdownMenu>
