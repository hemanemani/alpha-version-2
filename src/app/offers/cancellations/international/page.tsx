...
const CancellationsInternationalOffersDashboard:React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      const response = await axiosInstance.get<InternationalOffer[]>('/offer-international-cancellations', {
  ...
  const handleUpdateStatus = async (id: number,  status: number, offers_status: number
  ): Promise<void> => {
    try {
      const token = localStorage.getItem("authToken");
  ...
  const handleEdit = (id: number, index:number) => {
    ...
    router.push(`/inquiries/international/edit/${id}?serial=${serial}`);
  };

  const handleOffers = (id: number) => handleUpdateStatus(id, 1, 2);
  const handleBlockInternationalOffer = async (id: number, mobile_number: string, 
  ): Promise<void> =>  {
    try {
      const token = localStorage.getItem("authToken");
      ...
      const response = await axiosInstance.post<BlockResponse>(`/block-international-offer/${id}`, { mobile_number },
        { headers: 
   ...
   const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const filtered = inquiries.filter((row) => {
        const dateValue = row.international_offers?.[0]?.offer_date;
        
        // Format inquiry_date to DD-MM-YYYY
        const formattedOfferDate = dateValue
        ...
        row.name,
        row.mobile_number,
        row.specific_product,
        row.international_offers?.[0]?.received_sample_amount,
        row.international_offers?.[0]?.offer_notes,
        row.user?.name,
      ];
  
  
        return (
        formattedOfferDate.includes(value) ||
  ...
  //   },
    ...
    {
      accessorFn: (row) => row.international_offers?.[0]?.received_sample_amount,
    ...
    {
      accessorFn: (row) => row.international_offers?.[0].offer_notes,
      id: "offerNotes",
      header: "Notes",
      size: 200,
      cell: ({ row }) => {
      ...
      (accessLevel === "master") || accessLevel == "full" || accessLevel == "limited") && (
        <DropdownMenu open={openId === row.original.id} onOpenChange={(isOpen) => setOpenId(isOpen ? row.original.id : null)}>
          ...
          <DropdownMenuContent align="end" className="w-52 bg-white dark:bg-[#111111] border border-[#d9d9d9] dark:border-[#2e2e2e] rounded-lg">
            <DropdownMenuItem className="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-white cursor-pointer border-b border-b-[#d9d9d9] dark:border-b-[#2e2e2e] rounded-sm py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleEdit(id,serial)}>
            ...
            <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleOffers(row.original.id)}>
              <Move className="h-4 w-4 text-gray-600 dark:text-white" /> Move back to Offers
            </DropdownMenuItem>
            <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={() => handleBlockInternationalOffer(row.original.id,row.original.mobile_number)}>
  ...
  const table = useReactTable({
    ...
    const exportToCSV = () => {
      ...
      utils.book_append_sheet(workbook, worksheet, "Cancelled International Offers");
      writeFile(workbook, "cancelled-international-offers.csv");
    };
    ...
    const exportToExcel = () => {
      ...
      const workbook = utils.book_new();
      utils.book_append_sheet(workbook, worksheet, "Cancelled International Offers");
      writeFile(workbook, "cancelled-international-offers.xlsx");
    };
    ...
    const exportToPDF = () => {
      ...
      doc.save('cancelled-international-offers.pdf');
    };
        ...
        <div className="flex space-x-2 mb-6">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-40 bg-white border border-[#d9d9d9] rounded-lg dark:bg-[#111111]">
              <DropdownMenuItem
                className="flex items-center gap-2 text-sm font-inter-semibold text-black cursor-pointer py-2 border-b border-b-[#d9d9d9] dark:border-b-[#2e2e2e] rounded-sm dark:hover:bg-[#2C2D2F] dark:text-white"
...
export default CancellationsInternationalOffersDashboard;
