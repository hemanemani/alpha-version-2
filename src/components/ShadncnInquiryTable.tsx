                ...
                <TableCell className="text-[14px] font-[500] text-black py-4">
                  <DropdownMenu open={openId === inquiry.id} onOpenChange={(isOpen) => setOpenId(isOpen ? inquiry.id : null)}>
                    ...
                    <DropdownMenuContent align="end" className="w-52 bg-white dark:bg-[#111111] border border-[#d9d9d9] rounded-lg" forceMount>
                      <DropdownMenuItem className="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-white cursor-pointer border-b border-b-[#d9d9d9] rounded-none py-2 dark:hover:bg-[#2C2D2F]" onClick={()=>handleEdit(inquiry.id)}
                        >
                        <Edit className="h-4 w-4 text-black"/> Edit Inquiry
                      </DropdownMenuItem>
                      <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]"
                        onClick={()=>handleOffers(inquiry.id)}>
                        <Move className="h-4 w-4 text-gray-600 dark:text-white" /> Move to Offers
                      </DropdownMenuItem>
                      <DropdownMenuItem className="flex items-center gap-2 text-sm font-inter-semibold text-gray-900 dark:text-white cursor-pointer py-2 dark:hover:bg-[#2C2D2F]" onClick={()=>handleCancel(inquiry.id)}>
